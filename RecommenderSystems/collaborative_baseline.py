...
baseline_user = {}           #holds mean rating of each user

for user in range(len(data_matrix)):
    total_movie_ratings = 0
    for movie in range(len(data_matrix[user])):
        if data_matrix[user][movie]!=0:
            total_movie_ratings+=1
    mean = float(sum(data_matrix[user]))/total_movie_ratings
    baseline_user.update({user+1:mean})
...
baseline_movie = {}         #holds mean rating of each movie

total_no_ratings = 0        #total number of all ratings

total_ratings = 0           #sum of ratings of all the movies    
...
for movie in range(len(data_matrix)):
    ...
    if sum(data_matrix[movie]) == 0:
        ...
        mean = float(sum(data_matrix[movie]))/total_user_ratings

    total_ratings += sum(data_matrix[movie])
    total_no_ratings += total_user_ratings

    baseline_movie.update({movie+1: mean})                   #store mean movie ratings
...
global_mean = float(total_ratings)/total_no_ratings             #global mean
        ...
        for userID in range(len(test_matrix[movieID])):          #for each userID in test ratings for movieID
            if test_matrix[movieID][userID]!=0:                  #predicting for non-zero entries in test matrix
                ...
                if original[movieID][userID]==0:                  #predicting for missing entries in original i.e. data to 
                    ...
                    numerator = 0
                    denominator = 0
                    temp_similarity.pop(movieID+1)
                    while k_temp != 0:
                        m = max(temp_similarity.keys(), key=(lambda key: float(temp_similarity[key])))
                        if m-1 != movieID and original[m-1][userID] != 0:
                            bj  = baseline_movie[m]              # Average rating for movie m
                            bxj = bx + bj - global_mean             #baseline bxj to be subtracted from each value
                    ...
                    bi = baseline_movie[movieID+1]            #Global baseline added to the predicted value:bxi

                    bxi = bx + bi - global_mean
...
print('Collaborative Filtering with baseline done')